...
# create a time sequence of years 1-8
time_yr <- seq(from = 0, to = 8, by = 1) 

# create an empty matrix to store your population sizes after each year
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# store the initial population size in the first row
proj_year[1, ] <- fish_year0

# The for loop below is performing the dot product ad storing them into the matrix
